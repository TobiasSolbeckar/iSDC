...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// Calculate the normalization factor (i.e. 1/sum).
	float sum = 0;
	...
	// Multiply all elements in grid with (1/sum).
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	// For readability, and to make it easier to translate from the python function:
	...
	for(int i = 0;i < height;i++){
		for(int j = 0;j < width;j++){
			// Store the current grid value.
			grid_val = grid[i][j];
			for(int dx = -1;dx < 2;dx++){
				for(int dy = -1;dy < 2;dy++){
					// Multiply the current grid value with the corresponding window value.
					mult = window[dx+1][dy+1];
					// Due to difference in how mod is used in Python and c++, 'abs' needs to be used here.
					new_i = abs((i + dy) % height);
					new_j = abs((j + dx) % width);
					// Save the blurred value to the new grid.
					newGrid[new_i][new_j] += mult * grid_val;
				}
			}
		}
	}
	// Normalize and return the new grid.
