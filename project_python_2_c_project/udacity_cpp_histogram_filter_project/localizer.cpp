...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	// Calculate the normalization factor for the uniform distribution
	float value = 1.0 / (grid.size()*grid[0].size());
	// Create a new grid, and initialize it to 1 over the normalization factor
	vector< vector <float> > newGrid(grid.size(),vector<float>(grid[0].size(),value));
	return newGrid;
}
...
vector< vector <float> > move(int dy, int dx, vector < vector <float> > beliefs, float blurring) {
	// Create a new grid, with the same dimensions as 'beliefs'
	...
	for(int i = 0;i < beliefs.size();i++){
		...
		for(int j = 0;j < row.size();j++){
			cell = row[j];
			new_i = (i + dy) % beliefs.size();
			new_j = (j + dx) % beliefs[0].size();
